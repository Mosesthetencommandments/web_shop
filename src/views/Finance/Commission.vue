  ...
  <div class="app-container">
    ...
    <template>
      <el-table :key="tableKey" :data="list" v-loading="listLoading" element-loading-text="给我一点时间" stripe fit highlight-current-row border style="width: 100%;">
        ...
        <el-table-column label="提现金额" min-width="80" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="审核人" min-width="150" align="center">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.admin_nickname !=null" type="danger">{{scope.row.admin_nickname}}</el-tag>
          </template>
        </el-table-column>
...
<script>
  import { getShopDistributionProfitWithdrawRecordList, passShopDistributionProfitWithdrawRecord, noPassShopDistributionProfitWithdrawRecord, findShopDistributionProfitWithdrawRecordInfo } from '@/api/financial'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getShopDistributionProfitWithdrawRecordList(this.listQuery).then(response => {
      ...
      find(row) {
        this.shop_distribution_profit_withdraw_record_id = row.id
        this.dialogFind = true
        findShopDistributionProfitWithdrawRecordInfo({ shop_distribution_profit_withdraw_record_id: row.id }).then(response => {
      ...
      adopt(row) {
        ...
        this.resetemp()
        this.tempReject.shop_distribution_profit_withdraw_record_id = row.id
        console.log(this.tempReject.shop_distribution_profit_withdraw_record_id)
        this.$nextTick(() => {
      ...
      adoptData(row) {
        this.$refs['dateFormReject'].validate((valid) => {
          if (valid) {
            ...
            passShopDistributionProfitWithdrawRecord(this.tempReject).then(response => {
              this.getList()
      ...
      rejectData() {
        this.$refs['dateFormReject'].validate((valid) => {
          if (valid) {
            this.btnLoading = true
            noPassShopDistributionProfitWithdrawRecord(this.tempReject).then(response => {
              this.getList()
              this.dialogReject = false
              this.$notify({
                title: '成功',
